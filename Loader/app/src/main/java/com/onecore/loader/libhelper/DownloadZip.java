...
public class DownloadZip {
    ...
    public void startDownload(String downloadUrl, DownloadCallback callback) {
        com.onecore.loader.utils.DiagnosticLogger.log("DownloadZip.startDownload url=" + downloadUrl);
        ...
        executor.execute(() -> {
            ...
            com.onecore.loader.utils.DiagnosticLogger.log("Download finished success=" + success + " bytes=" + downloadedBytes);
            ...
            handler.post(() -> {
                if (success) {
                    ...
                    if (unzipEncrypted(zipPath, outputDir, password)) {
                        ...
                        com.onecore.loader.utils.DiagnosticLogger.log("ZIP extraction failed path=" + zipPath);
                    ...
                    com.onecore.loader.utils.DiagnosticLogger.log("Native ZIP download returned false");
    ...
    private boolean downloadFile(String downloadUrl, DownloadCallback callback) {
        ...
        try {
            ...
            connection.connect();
            com.onecore.loader.utils.DiagnosticLogger.log("HTTP connected response=" + connection.getResponseCode());
            ...
            com.onecore.loader.utils.DiagnosticLogger.log("ZIP saved path=" + outputZip.getAbsolutePath() + " exists=" + outputZip.exists());
            return outputZip.exists();

        } catch (Exception e) {
            com.onecore.loader.utils.DiagnosticLogger.exception("Native ZIP download exception", e);
            return false;
        } finally {
            if (connection != null) {
    ...
    private void moveSoFiles(File loaderFolder) {
        ...
        com.onecore.loader.utils.DiagnosticLogger.log("Moving native libraries count=" + (files == null ? 0 : files.length));
        if (files != null) {
            for (File soFile : files) {
                try {
                    ...
                    com.onecore.loader.utils.DiagnosticLogger.exception("Native library move failed", e);
                }
            }
        }
    }
    ...
    private void setPermissions(File fileOrDir) {
        ...
        try {
            ...
            com.onecore.loader.utils.DiagnosticLogger.exception("Permission update failed", e);
        }
    }
}
